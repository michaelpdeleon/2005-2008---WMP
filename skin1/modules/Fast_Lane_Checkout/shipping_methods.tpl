...
<font class="wwmp_shippingpayment_label">{$lng.lbl_shipping_address}</font>
<br /><br />
<!-- Deleted by Michael de Leon 11.09.06
{* include file="customer/main/subheader.tpl" title=$lng.lbl_shipping_address *}
-->
{if $userinfo}
<!-- Start of edit by Michael de Leon 11.09.06 -->
{$userinfo.b_address}<br /> 
{$userinfo.b_city}, {$userinfo.b_statename}<br />
{$userinfo.b_zipcode}<br />
{$userinfo.b_countryname}<br />
<!-- End of edit by Michael de Leon 11.09.06 -->
{else}
No data
{/if}
...
<!-- Delete by Michael de Leon 11.09.06
...
<font class="wwmp_shippingpayment_label">{$lng.lbl_delivery}</font>
<br /><br />
<!-- Deleted by Michael de Leon 11.09.06
...
<tr>
	...
	<td><label for="shippingid{$s.shippingid}">{$s.shipping|trademark:$insert_trademark}{if $s.shipping_time ne ""} - {$s.shipping_time}{/if}{if $config.Appearance.display_shipping_cost eq "Y" and ($login ne "" or $config.General.apply_default_country eq "Y" or $cart.shipping_cost gt 0)} (<font class="wwmp_shippingprice">{include file="currency.tpl" value=$s.rate}</font>){/if}</label></td>
</tr>
{if $s.warning ne ""}
<tr>
	<td>&nbsp;</td>
