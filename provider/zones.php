...
#
# $Id: zones.php,v 1.18 2006/01/11 06:56:25 mclap Exp $
#
...
#
	...
	#
		if (func_query_first_cell("SELECT countyid FROM $sql_tbl[counties] LIMIT 1")) {
			$zone_counties = func_query("SELECT $sql_tbl[counties].* FROM $sql_tbl[counties], $sql_tbl[zone_element] WHERE $sql_tbl[zone_element].field_type='G' AND $sql_tbl[zone_element].field=$sql_tbl[counties].countyid AND $sql_tbl[zone_element].zoneid='$zoneid'");

			$rest_counties = func_query("SELECT $sql_tbl[counties].*, $sql_tbl[zone_element].zoneid FROM $sql_tbl[states], $sql_tbl[countries], $sql_tbl[counties] LEFT JOIN $sql_tbl[zone_element] ON $sql_tbl[zone_element].field_type='G' AND $sql_tbl[zone_element].field=$sql_tbl[counties].countyid AND $sql_tbl[zone_element].zoneid='$zoneid' WHERE $sql_tbl[states].stateid=$sql_tbl[counties].stateid AND $sql_tbl[countries].code=$sql_tbl[states].country_code AND $sql_tbl[countries].active='Y' AND zoneid IS NULL ORDER BY $sql_tbl[states].country_code, $sql_tbl[states].state");
