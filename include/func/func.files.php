...
#
# $Id: func.files.php,v 1.22.2.2 2006/06/30 07:40:03 max Exp $
#
...
function func_get_image_url($id, $type = 'T', $image_path = false) {
	...
	if (!strncmp($xcart_dir, $image_path, strlen($xcart_dir)) && @file_exists($image_path)) {
		...
		return $current_location.str_replace(DIRECTORY_SEPARATOR, "/", substr($image_path, strlen($xcart_dir)));
	}
...
function func_allowed_path($allowed_path, $path) {
	if (empty($allowed_path) || empty($path)) return false;
