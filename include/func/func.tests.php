...
#
# $Id: func.tests.php,v 1.10.2.1 2006/05/10 05:58:00 max Exp $
#
...
#
# Function to test VeriSign PayFlow Pro client
# at this moment only linux client is tested
#
function test_payflow() {
	global $xcart_dir;
	if (!X_DEF_OS_WINDOWS) {
	...
	$file = func_normalize_path($xcart_dir."/payment/bin/pfpro".((X_DEF_OS_WINDOWS)? ".exe" : ""));
	if (!file_exists($file)) return "";

	$tmpfn = func_temp_store("");
	@exec(func_shellquote($file)." 2>".func_shellquote($tmpfn), $output);
	@unlink($tmpfn);
	if (!empty($output))
		return $output[0];
	return "";
...
function get_ccrequirements($module_params) {
	...
	switch ($module_params["processor"]) {
		...
		case 'ps_paypal_pro.php':
			...
			break;
		case 'cc_verisign.php':
			$result["testexec"] = $xcart_dir."/payment/bin/pfpro".((X_DEF_OS_WINDOWS)? ".exe" : "");
			$result["testfunc"] = "test_payflow";
			break;
