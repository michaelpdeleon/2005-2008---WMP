...
#
# $Id: import_tax_rates.php,v 1.14.2.1 2006/07/20 08:25:24 max Exp $
#
...
#
	...
	$import_specification["TAX_RATES"] = array(
		...
		"need_provider" => 1,
		"parent"		=> "TAXES",
		"export_sql"	=> "SELECT taxid FROM $sql_tbl[tax_rates]",
		"columns"		=> array(
...
#
	...
	if ($import_file["drop"]["tax rates"] == "Y") {
	#
	...
	foreach ($data_row as $tax_rate) {
		...
		$zoneid = 0;
		if (!empty($values["zone"]))
			$zoneid = func_import_get_cache("Z", $values["zone"]);
