...
#
# $Id: import_featured_products.php,v 1.5.2.1 2006/06/15 07:01:23 max Exp $
#
...
if ($import_step == "define") {
	...
	$import_specification['FEATURED_PRODUCTS'] = array(
		...
		"permissions"	=> "A",
		"parent"		=> "PRODUCTS",
		"export_sql"	=> "SELECT productid FROM $sql_tbl[featured_products]",
...
# Export data	
	...
	while ($id = func_export_get_row($data)) {
		...
		$row = func_query_first("SELECT * FROM $sql_tbl[featured_products] WHERE productid = '$id'");
		if (empty($row))
			continue;
