...
# Start
function get_subcat($id, $cats) {
if (empty($id) or !is_array($cats))
return "";
$id = intval($id);
$result = array();
foreach ($cats as $c) {
...
uasort($result, "func_sort_by_pos");
return $result;
};
function func_sort_by_pos($a, $b) {
...
$_categories = func_get_categories_list($cat, true, "all");
$_categories = $_categories[all_categories];
if (is_array($categories) && !empty($categories)) {
foreach ($categories as $i=>$c) {
$categories[$i][subcats] = get_subcat($c[categoryid],$_categories);
foreach ($categories[$i][subcats] as $k=>$sc) {
$categories[$i][subcats][$k][subcats] = get_subcat($sc[categoryid],$_categories);
foreach ($categories[$i][subcats][$k][subcats] as $l=>$ssc) {
$categories[$i][subcats][$k][subcats][$l][subcats] = get_subcat($ssc[categoryid],$_categories);
}
}
}
unset ($_categories);
