...
#
# $Id: taxes.php,v 1.29.2.2 2006/07/31 13:58:36 svowl Exp $
#
...
if ($REQUEST_METHOD == "POST") {
	...
	elseif ($mode == "delete_rates" && !empty($taxid)) {
		...
		#
		if (!empty($to_delete) && is_array($to_delete)) {
			db_query("DELETE FROM $sql_tbl[tax_rates] WHERE rateid IN ('".implode("','", array_keys($to_delete))."')");
...
if ($mode == "add" || !empty($taxid)) {
	...
	if (empty($tax_details)) {
		...
		if (!empty($provider_condition))
			$provider_condition_rates = preg_replace("/AND provider=/", "AND $sql_tbl[tax_rates].provider=", $provider_condition);
